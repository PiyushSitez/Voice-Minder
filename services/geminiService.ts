...
export const speakNative = (text: string, mood: 'calm' | 'urgent' | 'cheerful', speed: number = 1.0, voiceId?: string): Promise<void> => {
    return new Promise((resolve, reject) => {
        ...
        // Attempt to match requested voiceId (Gender based) with available browser voices
        ...
        if (voiceId) {
            const selectedOption = VOICE_OPTIONS.find(v => v.id === voiceId);
            if (selectedOption) {
                const targetGender = selectedOption.gender; // 'Male' or 'Female'
                
                // Try to find a voice matching the gender description in the browser
                // Note: Browser voice names vary by OS (e.g. "Microsoft David" is Male, "Google UK English Female")
        ...
        // Fallback priority if precise gender match failed
        if (!targetVoice) {
             targetVoice = voices.find(v => v.name.includes('Google') || v.name.includes('Natural')) || voices[0];
        }
