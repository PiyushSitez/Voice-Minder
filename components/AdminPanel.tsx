...
import { Lock, Check, X, Trash2, Send, ShieldAlert, Users, DollarSign, MessageCircle, Zap, Ban, RotateCcw, AlertTriangle, LayoutList, Settings, History, Image, Clock, Calendar, CheckSquare, Square, Paperclip, FileText, Image as ImageIcon, Download, Upload, Database, Wallet, TrendingUp, ArrowUpRight } from 'lucide-react';
...
const AdminPanel: React.FC<AdminProps> = ({ onBack }) => {
  ...
  const [transactionSubTab, setTransactionSubTab] = useState<'pending' | 'history' | 'gallery' | 'balance'>('pending');
  ...
  const confirmBulkDelete = async () => {
      ...
      const idsToRemove = new Set(selectedTxIds);
      ...
      const deletePromises = Array.from(idsToRemove).map(id => Storage.deleteTransaction(id));
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Changed from (event) => event.target.result to () => reader.result to fix TS error
      reader.onload = async () => {
          const json = reader.result;
          if (typeof json === 'string') {
              const success = await Storage.importDatabase(json);
              if(success) {
  ...
  const totalBalance = approvedTransactions.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  return (
    <div className="glass-card rounded-[2rem] shadow-2xl min-h-[80vh] border border-red-900/30 overflow-hidden flex flex-col animate-in fade-in duration-500 relative">
      ...
      {bulkDeleteModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
              <div className="glass-card p-8 max-w-md w-full text-center border border-red-500/50 shadow-[0_0_50px_rgba(220,38,38,0.5)] animate-in zoom-in-95 rounded-3xl">
                  <div className="w-16 h-16 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Trash2 size={32} className="text-red-500" />
                  </div>
                  <h3 className="text-2xl font-bold text-white mb-2">Delete {selectedTxIds.size} Items?</h3>
                  <p className="text-gray-400 mb-6 text-sm">This action cannot be undone and will permanently remove selected records.</p>
                  
                  <div className="text-left mb-2 text-xs font-bold text-red-400 uppercase">Type "DELETE" to confirm</div>
                  <input 
                    type="text" 
                    value={bulkDeleteConfirmText}
                    onChange={(e) => setBulkDeleteConfirmText(e.target.value)}
                    className="w-full bg-black border border-red-900 rounded-xl p-3 text-white mb-6 focus:border-red-500 outline-none placeholder-gray-600 font-mono tracking-widest text-center"
                    placeholder="DELETE"
                  />
                  ...
                  <div className="flex gap-3">
                      ...
                      <button 
                        ...
                        className={`flex-1 py-3 rounded-xl font-bold text-white transition-all ${bulkDeleteConfirmText === "DELETE" ? 'bg-red-600 hover:bg-red-500 hover:shadow-lg shadow-red-500/50' : 'bg-red-900/30 cursor-not-allowed opacity-50'}`}
                      >
                          Confirm Delete
                      </button>
                  </div>
        ...
        {activeTab === 'transactions' && (
          <div className="space-y-6 animate-in slide-in-from-right-5 duration-300">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-4 p-1 bg-white/5 rounded-xl backdrop-blur-md border border-white/10 w-full md:w-auto inline-flex overflow-x-auto scrollbar-hide">
                ...
                <button 
                    onClick={() => setTransactionSubTab('balance')}
                    className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-2 whitespace-nowrap ${transactionSubTab === 'balance' ? 'bg-green-600 text-white shadow-lg shadow-green-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                >
            ...
            {transactionSubTab === 'balance' && (
                <div className="animate-in fade-in slide-in-from-bottom-2 space-y-8">
                     <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <h3 className="font-bold text-xl text-white flex items-center gap-3">
                            <div className="w-1 h-8 bg-green-500 rounded-full"></div> Balance & Revenue
                        </h3>
                        <span className="text-xs text-gray-400 bg-black/40 px-3 py-1 rounded-full border border-white/10">
                            Updates automatically on approval
                        </span>
                     </div>

                     {/* Main Balance Card */}
                     <div className="glass-card rounded-[2.5rem] p-8 md:p-12 text-center border border-green-500/30 bg-gradient-to-b from-green-900/10 to-black relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            ...
                            <h4 className="text-sm font-bold text-green-500 uppercase tracking-widest mb-2">Total Approved Revenue</h4>
                            <div className="text-6xl md:text-8xl font-black text-white tracking-tighter drop-shadow-2xl flex items-start justify-center gap-2">
                                <span className="text-2xl md:text-4xl text-gray-500 mt-2 md:mt-4">₹</span>
                                {totalBalance.toLocaleString()}
                            </div>
                            <p className="mt-4 text-gray-400 text-sm max-w-md">
                                This amount reflects the sum of all transactions that have been marked as <span className="text-green-400 font-bold">APPROVED</span>.
                            </p>
                        </div>
                     </div>

                     {/* Transactions List */}
                     <div className="glass-card rounded-[2rem] p-6 border border-gray-800">
                        <h4 className="font-bold text-gray-300 mb-6 flex items-center gap-2">
                            <ArrowUpRight size={18} className="text-green-500" /> Incoming Payments Log
                        </h4>
                        
                        <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                            {approvedTransactions.length === 0 && (
                                <div className="text-center py-10 text-gray-500 italic">No approved payments yet.</div>
                            )}
                            {approvedTransactions.map(tx => (
                                <div key={tx.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5 hover:bg-green-900/10 transition group">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-green-900/40 flex items-center justify-center text-green-400 border border-green-500/30 font-bold">
                                            {getUserNameByEmail(tx.userEmail).charAt(0)}
                                        </div>
                                        <div>
                                            <p className="font-bold text-white">{getUserNameByEmail(tx.userEmail)}</p>
                                    ...
                                    <div className="flex items-center gap-6 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">
                                        <div className="text-right">
                                            ...
                                            <p className="text-sm text-gray-300">{new Date(tx.date).toLocaleDateString()}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] text-gray-500 uppercase font-bold">Plan</p>
                                            <p className="text-sm text-yellow-500 font-bold">{tx.plan}</p>
                                        </div>
                                        <div className="text-right min-w-[80px]">
                                            <p className="text-[10px] text-gray-500 uppercase font-bold">Amount</p>
                                            <p className="text-xl font-bold text-green-400">+₹{tx.amount}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
