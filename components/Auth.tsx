...
interface AuthProps {
  ...
  onSwitchMode: () => void; // New prop to handle switching pages
}
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'signup') {
      ...
      const newUser: User = {
        ...
        isTrialEligible: true // New users are eligible for the 1 Hour Deal
      };
      ...
      <div className="max-w-md w-full space-y-8 glass-card p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden shadow-[0_0_60px_rgba(0,0,0,0.6)] animate-in zoom-in-95 duration-500 border border-white/10">
        ...
        <form className="mt-8 md:mt-10 space-y-6 relative z-10" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              className="group relative w-full flex justify-center py-4 px-4 border border-transparent text-base font-bold rounded-2xl text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-[0_0_25px_rgba(139,92,246,0.4)] transition-all hover:scale-[1.02] hover:shadow-[0_0_35px_rgba(139,92,246,0.6)]"
            >
              {type === 'login' ? t('auth_btn_login') : t('auth_btn_signup')}
            </button>
          </div>
        </form>
        ...
        <div className="text-center mt-6 relative z-10">
             <button
                type="button" // Ensure this is type button so it doesn't submit form
