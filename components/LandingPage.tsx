...
const LandingPage: React.FC<LandingPageProps> = ({ onNavigate, onSelectPlan }) => {
  ...
  useEffect(() => {
    ...
    const stars: Star[] = [];
    const BASE_STARS = 800; // High density to ensure full coverage
    ...
    class Star {
      ...
      constructor() {
        // Initialize random position across full window width
        this.x = Math.random() * width;
        this.y = Math.random() * height;
        this.size = Math.random() * 2;
        // Increased speed for better movement visibility
      ...
      update() {
        ...
        // Wrap around screen logic to keep stars present everywhere
    ...
    const initStars = () => {
        stars.length = 0;
        // Use current width to populate stars
        const currentW = window.innerWidth;
        const currentH = window.innerHeight;
        
        for (let i = 0; i < BASE_STARS; i++) {
          const s = new Star();
          // Explicitly set random x based on current width during init
          s.x = Math.random() * currentW;
          s.y = Math.random() * currentH;
          stars.push(s);
        }
    };
    initStars();
    ...
    const handleResize = () => {
      setCanvasSize();
      // Important: Re-initialize to fill new space if window grew
      initStars(); 
    };
  ...
  return (
    <div className="text-white overflow-x-hidden bg-black relative w-full">
      ...
      {/* Hero Section */}
      <section 
        className="relative flex flex-col items-center justify-center min-h-[70vh] md:min-h-screen w-full pt-16 pb-10 md:pt-20"
      ...
      >
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10 text-center perspective-1000 flex flex-col items-center">
          ...
          {/* Magnetic Title */}
          <div 
            className="transition-transform duration-100 ease-out mb-4 md:mb-8 w-full max-w-5xl"
          ...
          >
            ...
            >
                {/* Mobile Text Size Increased significantly (7xl base for mobile) */}
                <span className="block text-7xl sm:text-7xl md:text-8xl lg:text-9xl bg-clip-text text-transparent bg-gradient-to-b from-white via-gray-200 to-gray-500 py-2">{t('hero_title1')}</span>
                <span className="block text-7xl sm:text-7xl md:text-8xl lg:text-9xl bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300 drop-shadow-[0_0_50px_rgba(168,85,247,0.5)] py-2">
                {t('hero_title2')}
                </span>
            </h1>
          </div>
      ...
      <section className="py-12 md:py-24 relative border-t border-white/5 bg-gradient-to-b from-black to-indigo-950/20 reveal">
