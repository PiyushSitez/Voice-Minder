...
import { OWNER_EMAIL, VOICE_OPTIONS, CATEGORIES } from '../constants';
import { Clock, Play, Trash2, Plus, Settings, Volume2, Bell, Zap, X, Calendar, Mic, CheckCircle, Repeat, Activity, Music, Smile, TrendingUp, CalendarDays, Layout, BarChart3, AlertOctagon, FastForward, CheckSquare, List, Trash, Lock, Timer, Crown, PartyPopper, Tag, RefreshCcw, Sun, Smartphone } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user: initialUser, onUpgrade }) => {
  ...
  const [voiceId, setVoiceId] = useState('en-IN-Female'); // Default to India Female
  ...
  const requestWakeLock = async () => {
    ...
    try {
        ...
        if (err.message && err.message.includes('permissions policy')) {
             console.warn("Wake Lock disabled by permissions policy.");
        ...
        // Silently ignore NotAllowedError (tab hidden or denied)
    }
  };
  ...
  useEffect(() => {
    ...
    // Re-request if visibility changes (user comes back to tab)
    ...
    // CHECK FOR DEFAULT NOTIFICATION PERMISSION TO SHOW POPUP
    if ("Notification" in window && Notification.permission === 'default' && !sessionStorage.getItem('notif_modal_shown')) {
         const timer = setTimeout(() => setShowPermissionModal(true), 1500); // Slight delay for better UX
         sessionStorage.setItem('notif_modal_shown', 'true'); // Prevent showing again in same session if ignored
         return () => {
             clearTimeout(timer);
  ...
  const requestNotificationPermission = async () => {
      ...
      if (Notification.permission === 'denied') {
        alert("Notifications are blocked! Please click the Lock icon in your address bar and Allow 'Notifications' manually.");
        return;
      }
      ...
      try {
        ...
        if(perm === 'granted') {
            ...
            new Notification("VoiceMinder", { body: "Background alerts enabled!", icon: '/logo192.png' });
        }
      } catch (e) {
    ...
    pending.forEach(r => {
      ...
      if (rTime <= now && (now.getTime() - rTime.getTime()) < 60000 * 5) { 
         if(notification?.id !== r.id) {
             ...
             // FIRE SYSTEM NOTIFICATION (For when screen is off)
             if (Notification.permission === 'granted') {
                 new Notification(r.subject, {
                     body: r.text,
                     icon: '/logo192.png', // Assuming public path
                     requireInteraction: true,
                     silent: false
                 });
             }

             // Logic change: don't mark completed immediately for UI purposes,
             // wait for dismiss. But technically it triggered.
             // We'll mark as completed to prevent double triggering in this session loop
             // but handle recurrence in handleDismissNotification
             markAsCompleted(r);
         }
      }
    });
  ...
  const markAsCompleted = async (r: Reminder) => {
    ...
    if (r.recurrence && r.recurrence !== 'none') {
        const oldTime = new Date(r.time);
        let nextTime = new Date(oldTime);
        
        if (r.recurrence === 'daily') nextTime.setDate(oldTime.getDate() + 1);
        if (r.recurrence === 'weekly') nextTime.setDate(oldTime.getDate() + 7);
        
        // Ensure next time is in future (in case system was off for a week)
        const now = new Date();
        while (nextTime <= now) {
                if (r.recurrence === 'daily') nextTime.setDate(nextTime.getDate() + 1);
                else if (r.recurrence === 'weekly') nextTime.setDate(nextTime.getDate() + 7);
        }

        const updated = { ...r, time: nextTime.toISOString() };
  ...
  const playNativeLoop = async (fullText: string, r: Reminder) => {
      const loop = async () => {
          ...
          // Pass the voiceId and volume to speakNative
          await GeminiService.speakNative(fullText, r.mood, r.speed, r.voiceId, r.volume || 1.0);
  ...
  const playReminderLoop = async (r: Reminder) => {
    ...
    const startAudioLoopWithData = (audioData: string) => {
        const playWithGap = async () => {
            ...
            const audioResult = await GeminiService.playAudioFromBase64(audioData, r.speed, false, vol);
    ...
    if (fullText.length > 100) {
        try {
            ...
            if (introAudio && isNotificationActiveRef.current) {
                const introResult = await GeminiService.playAudioFromBase64(introAudio, r.speed, false, vol);
  ...
  const handleSnoozeNotification = async () => {
      ...
      if(notification) {
          ...
          // We create a temp snooze reminder (or update existing one if needed logic)
          // For simplicity, we overwrite current reminder time, but preserve original recurrence logic
  ...
  const handlePreviewVoice = async () => {
      ...
      if (!GeminiService.hasApiKey()) {
          // Pass voiceId and volume
          await GeminiService.speakNative(previewText, mood, speed, voiceId, volume);
      ...
      if (audioData) {
          GeminiService.playAudioFromBase64(audioData, speed, false, volume);
      } else {
           await GeminiService.speakNative(previewText, mood, speed, voiceId, volume);
      }
  };
  ...
  const handleAddReminder = async (e: React.FormEvent) => {
    ...
    // Auto-request wake lock and notifications on user interaction (form submit)
    if(!isWakeLockActive) requestWakeLock();
    // Only request if not explicitly denied or granted, or let the browser handle it
  ...
  let totalSpeed = 0;
  reminders.forEach(r => {
    const vId = r.voiceId || 'en-IN-Female';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 relative">
      ...
      {/* Wake Lock & Notification Status Bar */}
      <div className="col-span-full flex flex-wrap gap-2 items-center justify-between bg-black/40 p-3 rounded-xl border border-white/5">
        <div className="flex items-center gap-4">
             {isWakeLockActive ? (
                 <div className="flex items-center gap-2 text-green-400 text-xs font-bold bg-green-900/20 px-3 py-1.5 rounded-full border border-green-900/50">
                     <Sun size={14} className="animate-pulse" /> {t('perm_wakelock_active')}
                 </div>
             ) : (
                 wakeLockSupported && (
                    <button onClick={requestWakeLock} className="flex items-center gap-2 text-yellow-400 text-xs font-bold bg-yellow-900/20 px-3 py-1.5 rounded-full border border-yellow-900/50 hover:bg-yellow-900/40">
                        <Sun size={14} /> Enable Screen Lock
                    </button>
                 )
             )}
             
             {notificationPermission !== 'granted' && (
                 <button onClick={requestNotificationPermission} className="flex items-center gap-2 text-indigo-400 text-xs font-bold bg-indigo-900/20 px-3 py-1.5 rounded-full border border-indigo-900/50 hover:bg-indigo-900/40">
                     <Bell size={14} /> {t('perm_enable_notif')}
                 </button>
             )}
        </div>
      </div>
                    ...
                    <form onSubmit={handleAddReminder} className="relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div className="space-y-5">
                                ...
                                <LockedFeature isLocked={advLock.isLocked} lockLabel={advLock.label} onUpgrade={onUpgrade}>
                                    <div>
                                        <label className="block text-xs font-bold text-indigo-300 uppercase tracking-wider mb-2 pl-1">{t('frm_category')}</label>
                                        <div className="flex flex-wrap gap-2">
                                            {CATEGORIES.map(cat => (
                                                <button
                                                    ...
                                                    className={`px-3 py-2 rounded-lg text-xs font-bold uppercase border transition-all flex items-center gap-1 ${category === cat.id ? `bg-${cat.color}-600 border-${cat.color}-400 text-white shadow-lg` : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10'}`}
                                                >
                                                    <Tag size={12} /> {t(cat.label)}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <LockedFeature isLocked={speedLock.isLocked} lockLabel={speedLock.label} onUpgrade={onUpgrade}>
                                        <div>
                                            <label className="block text-xs font-bold text-indigo-300 uppercase tracking-wider mb-2 pl-1">{t('frm_volume')}: {Math.round(volume * 100)}%</label>
                                            <input type="range" min="0.1" max="1.0" step="0.1" value={volume} onChange={e => setVolume(parseFloat(e.target.value))} className="w-full accent-green-500 cursor-pointer h-2 bg-gray-700 rounded-lg appearance-none" />
                                        </div>
                                    </LockedFeature>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                     <LockedFeature isLocked={advLock.isLocked} lockLabel={advLock.label} onUpgrade={onUpgrade}>
                                        <div>
                                            <label className="block text-xs font-bold text-indigo-300 uppercase tracking-wider mb-2 pl-1">{t('frm_recurrence')}</label>
                                            <div className="relative">
                                                <select 
                                                    className="w-full glass-input rounded-xl p-3 appearance-none cursor-pointer text-white bg-black text-xs font-bold"
                                                    value={recurrence}
                                                    onChange={e => setRecurrence(e.target.value as any)}
                                                >
                                                    <option value="none">{t('rec_none')}</option>
                                                    <option value="daily">{t('rec_daily')}</option>
                                                    <option value="weekly">{t('rec_weekly')}</option>
                                                </select>
                                                <RefreshCcw className="absolute right-3 top-3 text-gray-400 pointer-events-none" size={14} />
                                            </div>
                                        </div>
                                     </LockedFeature>
                                     
                                     <div className="flex flex-col justify-end">
                                        <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/10 h-[46px]">
                                            <label className="text-xs text-gray-300 font-medium flex items-center gap-2"><Repeat size={14} /> {t('frm_repeat')}</label>
                                            <button 
                                                type="button"
                                                onClick={() => setRepeatVoice(!repeatVoice)}
                                                className={`w-10 h-5 rounded-full transition-all relative ${repeatVoice ? 'bg-green-500' : 'bg-gray-600'}`}
                                            >
                                                <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${repeatVoice ? 'left-6' : 'left-1'}`}></div>
                                            </button>
                                        </div>
                                     </div>
                                </div>
                                ...
                                <div className="relative z-10">
                                    <div className="flex justify-between items-start">
                                        ...
                                        {nextUpcoming.category && (
                                            <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded bg-${CATEGORIES.find(c => c.id === nextUpcoming.category)?.color || 'gray'}-900 text-${CATEGORIES.find(c => c.id === nextUpcoming.category)?.color || 'gray'}-400 border border-${CATEGORIES.find(c => c.id === nextUpcoming.category)?.color || 'gray'}-700`}>
                                                {nextUpcoming.category}
                                            </span>
                                        )}
                                    </div>
                                ...
                                {ongoingReminders.map(r => (
                                    <div key={r.id} className="bg-white/5 p-3 rounded-lg border border-white/5 text-sm hover:bg-white/10 transition group relative">
                                        <div className="flex justify-between mb-1">
                                            ...
                                            {r.recurrence && r.recurrence !== 'none' && <RefreshCcw size={12} className="text-yellow-500" />}
                                        </div>
                                        <p className="text-gray-400 truncate">{r.subject}</p>
      ...
      {/* Notification Permission Modal (First Time Login) */}
      {showPermissionModal && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 animate-in fade-in duration-500">
              <div className="max-w-sm w-full glass-card rounded-[2.5rem] p-8 relative overflow-hidden border border-indigo-500/30 shadow-[0_0_80px_rgba(99,102,241,0.3)] animate-in zoom-in-50 duration-500 text-center">
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                      <div className="w-20 h-20 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center mb-6 shadow-lg animate-bounce">
                          <Bell size={40} className="text-white fill-white" />
                      </div>
                      
                      <h2 className="text-2xl font-bold text-white mb-2">{t('notif_modal_title')}</h2>
                      ...
                      <button 
                          ...
                          className="w-full py-3 bg-white text-black font-bold rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.4)] hover:scale-105 transition-all mb-3"
                      >
                          {t('notif_modal_allow')}
                      </button>
              ...
              <div className="relative bg-gray-900 rounded-[2.5rem] shadow-[0_0_100px_rgba(234,179,8,0.6)] p-8 md:p-10 max-w-md w-full text-center border border-yellow-500/50">
                  ...
                  <div className="relative z-10">
                    ...
                    {notification.category && (
                        <div className="mb-4">
                             <span className={`text-xs font-bold uppercase px-3 py-1 rounded-full bg-${CATEGORIES.find(c => c.id === notification.category)?.color || 'gray'}-900 text-${CATEGORIES.find(c => c.id === notification.category)?.color || 'gray'}-400 border border-${CATEGORIES.find(c => c.id === notification.category)?.color || 'gray'}-700`}>
                                {notification.category}
                            </span>
                        </div>
                    )}
