...
const Dashboard: React.FC<DashboardProps> = ({ user: initialUser, onUpgrade }) => {
  ...
  const playNativeLoop = async (fullText: string, r: Reminder) => {
      const loop = async () => {
          ...
          // Pass the voiceId to speakNative for correct fallback behavior
          await GeminiService.speakNative(fullText, r.mood, r.speed, r.voiceId);
  ...
  const handlePreviewVoice = async () => {
      ...
      if (!GeminiService.hasApiKey()) {
          // Pass voiceId here too
          await GeminiService.speakNative(previewText, mood, speed, voiceId);
