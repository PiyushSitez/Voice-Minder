...
const App: React.FC = () => {
  ...
  // Store the timestamp of when Admin last logged in successfully
  const [lastAdminVisit, setLastAdminVisit] = useState<number>(() => {
      const stored = localStorage.getItem('vm_admin_last_visit');
  ...
  useEffect(() => {
    // ASYNC CHECK
    const init = async () => {
        const checkUser = await Storage.getCurrentUser();
        if (checkUser) {
            setUser(checkUser);
  ...
  // Check for admin notifications based on LAST VISIT timestamp
  useEffect(() => {
      if (!user?.isAdmin) return;

      const checkAdminNotifications = async () => {
          const txs = await Storage.getTransactions();
          // Count only pending transactions that happened AFTER last visit
          const pendingTx = txs.filter(t => t.status === 'pending' && t.date > lastAdminVisit).length;
          
          const allChats = await Storage.getAllChats();
          // Count only unread chats that happened AFTER last visit
          const unreadMsgs = allChats.filter(m => m.receiverId === 'admin' && !m.read && m.timestamp > lastAdminVisit).length;

          setAdminBadgeCount(pendingTx + unreadMsgs);
      };

      checkAdminNotifications();
      const interval = setInterval(checkAdminNotifications, 5000); 

      return () => clearInterval(interval);
  }, [user, lastAdminVisit]); // Re-run when lastAdminVisit changes
  ...
  // Called by AdminPanel when PIN is entered correctly
  const handleAdminLogin = () => {
      const now = Date.now();
      setLastAdminVisit(now);
      localStorage.setItem('vm_admin_last_visit', now.toString());
      setAdminBadgeCount(0); // Clear badge immediately
  };
        ...
        <div key={currentPage} className="page-enter-active w-full">
            {(() => {
                ...
                if (currentPage === 'admin') {
                    ...
                    return <AdminPanel onBack={() => navigate('home')} onLoginSuccess={handleAdminLogin} />;
                }
                ...
                if (currentPage === 'profile') {
                    return <Profile user={user} onUpgrade={() => navigate('pricing')} onUserUpdate={setUser} />;
                }
  ...
  const showBackButton = currentPage !== 'landing' && currentPage !== 'home';
              ...
              <div className="ml-4 flex items-center space-x-1">
                ...
                {user ? (
                  <>
                    ...
                    {user.isAdmin && (
                        <button onClick={() => navigate('admin')} className={`relative px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-all hover:scale-105 ${currentPage === 'admin' ? 'text-red-400 bg-red-900/20 border border-red-500/30' : 'text-gray-300 hover:bg-white/5 hover:text-red-300'}`}>
                            <ShieldCheck size={16} /> {t('nav_admin')}
                            {adminBadgeCount > 0 && (
                                <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold rounded px-1 animate-pulse border border-black shadow-lg">
                                    {adminBadgeCount}
                                </span>
                            )}
                        </button>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              ...
              {user ? (
                  <>
                    ...
                    {user.isAdmin && (
                        <button onClick={() => navigate('admin')} className="block w-full text-left px-3 py-3 rounded-md text-base font-medium text-red-400 hover:bg-red-900/20 flex justify-between items-center">
                            {t('nav_admin')}
                            {adminBadgeCount > 0 && (
                                <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{adminBadgeCount}</span>
                            )}
                        </button>
                    )}
